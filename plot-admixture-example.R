...
files <- list.files(path = '~/qmatrix', pattern = "\\.Q$", full.names=T)
files <-files[order(nchar(files), files)]
fam_path <- list.files(path = '~/qmatrix', pattern = "\\.fam$")
...
continent_gr <- more_info[, c('continent_region','population'), drop =F]
...
plotQ(adm_results,imgoutput="join",
      ...
      ordergrp = T,panelratio=c(10,10),splab=labels_adm)
